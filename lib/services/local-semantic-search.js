 ...
 * 
 * The system loads chunks and embeddings from local files and performs
 * vector similarity search using cosine similarity between embeddings.
 */
...
const DEFAULT_CONFIG = {
  CHUNKS_PATH: path.join(PROJECT_ROOT, 'data', 'chunks.jsonl'),
  EMBEDDINGS_PATH: path.join(PROJECT_ROOT, 'data', 'embeddings.jsonl'),
  EMBEDDING_SCRIPT_PATH: path.join(PROJECT_ROOT, 'lib', 'embeddings', 'generate-embedding.js'),
  MAX_RESULTS: 5,
...
class LocalSemanticSearch {
  ...
  async generateEmbedding(text) {
    ...
    try {
      ...
      if (fs.existsSync(this.config.EMBEDDING_SCRIPT_PATH)) {
        // Use direct implementation instead of calling the script
        // This avoids any issues with console output mixing with JSON
        try {
          // Directly require and use the embedding generator module
          const embeddingGenerator = require('../embeddings/generate-embedding');
          const embedding = embeddingGenerator.generateEmbedding(text, this.config.EMBEDDING_DIMENSIONS);
          ...
          logger.warn(`Error using direct embedding generation: ${directError.message}, falling back to script execution`);
          
          // If direct import fails, try the script execution as before
          ...
          // Carefully extract only valid JSON from the result
          ...
          if (jsonStart >= 0 && jsonEnd > jsonStart) {
            ...
            try {
              ...
              logger.error(`Failed to parse embedding JSON: ${jsonError.message}`);
              // Continue to fallback
            }
          } else {
            logger.error(`Invalid embedding result format: ${result.substring(0, 100)}...`);
            // Continue to fallback
          }
        }
      } else {
    ...
    // Use fallback hash-based embedding for any failure case
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      if (validEmbedding && this.embeddings.size > 0) {
        ...
        eventBus.emit('semantic-search:completed', {
          query,
        ...
        logger.warn(`No embedding available for query, falling back to keyword search`);
        this._diagnostics.fallbackCount++;
        
        // Emit fallback event
        eventBus.emit('semantic-search:fallback', {
          query,
  ...
  async _semanticSearch(query, queryEmbedding, options = {}) {
    ...
    // Log the top 10 similarities regardless of threshold
    logger.debug('[Leo Debug] Top 10 similarity scores:');
    scored.slice(0, 10).forEach((item, idx) => {
      const title = item.chunk.title || item.chunk.path || (item.chunk.content ? item.chunk.content.slice(0, 60) : '(no title)');
      logger.debug(`#${idx + 1}: id=${item.chunk.id}, score=${item.similarity.toFixed(5)}, title=${title}`);
    });

    // Only filter by similarity threshold here
    const similarities = scored.filter(item => item.similarity >= similarityThreshold);

    // Sort again (should already be sorted)
  ...
  getDiagnostics() {
    return {
      ...
      embeddingsPath: this.config.EMBEDDINGS_PATH,
      chunksLoaded: this.chunks.length,
      embeddingsLoaded: this.embeddings.size,
      lastSearchTime: this._diagnostics.latestSearchTime,
      lastSearchQuery: this._lastQuery,
      lastSearchResults: this._lastResults ? this._lastResults.length : 0,
      cacheHits: this._diagnostics.cacheHits,
      cacheMisses: this._diagnostics.cacheMisses,
      fallbackCount: this._diagnostics.fallbackCount
    };
  }

  /**
   * Get a specific chunk by its ID
   * @param {string} id - The chunk ID to retrieve
   * @returns {Object|null} The chunk or null if not found
   */
  getChunkById(id) {
    if (!id) {
      logger.warn('getChunkById called with empty ID');
      return null;
    }
    
    const chunk = this.chunks.find(c => c.id === id);
    if (!chunk) {
      logger.warn(`Chunk with ID ${id} not found`);
      return null;
    }
