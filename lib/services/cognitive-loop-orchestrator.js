...
async function _preserveContext() {
  ...
  try {
    // Ensure semantic context manager is initialized if available
    if (semanticContextManager && !semanticContextManager.isInitialized && semanticContextManager.initialize) {
      try {
        logger.info('Initializing semantic context manager before context extraction');
        await semanticContextManager.initialize({
          configManager: options.configManager || options.configService, // Support both naming conventions
          eventBus: eventBus
        });
      } catch (initError) {
        logger.warn(`Failed to initialize semantic context manager: ${initError.message}`);
        // Continue anyway, might work with partial initialization
      }
    }
    
    // Extract critical context using a more robust approach
    let contextResult;
    if (semanticContextManager) {
      // Try multiple methods to extract context for better compatibility
      if (typeof semanticContextManager.extractCriticalContext === 'function') {
        contextResult = await semanticContextManager.extractCriticalContext();
      } else if (typeof semanticContextManager.extractContext === 'function') {
        contextResult = await semanticContextManager.extractContext();
      } else if (typeof semanticContextManager.getContext === 'function') {
        contextResult = { success: true, context: await semanticContextManager.getContext() };
      } else {
        logger.warn('No compatible context extraction method found on semantic context manager');
        contextResult = { success: false, error: 'No compatible extraction method' };
      }
      
      if (!contextResult || !contextResult.success) {
        logger.warn(`Failed to extract context: ${contextResult?.error || 'Unknown error'}`);
        // Create fallback minimal context
        contextResult = {
          success: true,
          context: {
            type: 'fallback',
            content: 'Minimal fallback context due to extraction failure',
            timestamp: Date.now()
          }
        };
      }
      ...
      // Preserve context
      if (contextPreservationSystem) {
        // Support multiple method signatures for better compatibility
        let preservationResult;
        if (typeof contextPreservationSystem.preserveContext === 'function') {
          preservationResult = await contextPreservationSystem.preserveContext({
            ...
            configManager: options.configManager || options.configService // Pass with CONSISTENT naming
          });
        } else if (typeof contextPreservationSystem.storeContext === 'function') {
          preservationResult = await contextPreservationSystem.storeContext(contextResult.context);
        } else {
          logger.warn('No compatible context preservation method found');
          preservationResult = { success: false, error: 'No compatible preservation method' };
        }
